...
export const ContractDetails: React.FC<ContractDetailsProps> = ({
  ...
  const handleSelectCategory = (categoryId: string) => {
    ...
    const displayName = (found as any)?.name || (found as any)?.label || currentCategory || "";

    setSelectedCategoryName(displayName);

    // Make sure we save the underlying ID/value so backend gets the correct category_id
    onContractChange("category", String((found as any)?.id ?? (found as any)?.value ?? categoryId));
  };
  ...
  useEffect(() => {
    ...
    if (contractForm.category && categories) {
      ...
      setSelectedCategoryName(
        (found as any)?.name || (found as any)?.label || currentCategory || ""
      );
    }
            ...
            >
              ...
              <View style={styles.section}>
                <FormField
                  ...
                  value={contractForm?.payments ?? ""}
                  onChangeText={(text) => {
                    onContractChange("payments", text);
                  }}
